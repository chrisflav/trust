...
exports.canonicalClaim = void 0;
exports.createRoutes = createRoutes;
const express_1 = require("express");
const certificate_1 = require("./certificate");
Object.defineProperty(exports, "canonicalClaim", { enumerable: true, get: function () { return certificate_1.canonicalClaim; } });
...
function createRoutes(config, store, auth) {
    ...
    routes.post('/api/tokens', requireUser, async (request, response) => {
        ...
        await store.createToken(currentUser(request).id, auth.hashToken(token), typeof name === 'string' ? name.slice(0, 100) : '');
    ...
    routes.post('/api/keys', requireUser, async (request, response) => {
        const { armored } = request.body;
        const key = await (0, certificate_1.readPublicKey)(armored);
        if ('error' in key) {
        ...
        const onGitHub = published.some((candidate) => candidate.replace(/\s/g, '') === armored.replace(/\s/g, ''));
        const verifiedVia = onGitHub ? 'github' : 'self';
        await store.upsertKey(user.id, key.fingerprint, armored, verifiedVia);
    ...
    routes.post('/api/certificates', requireUser, async (request, response) => {
        ...
        const claim = (0, certificate_1.parseClaim)(body.claim);
        if ('error' in claim) {
        ...
        if (typeof body.signature === 'string' && body.signature.length > 0) {
            const verdict = await (0, certificate_1.verifySignature)(claim, body.signature, await store.keysForIdentity(user.id));
            if (!verdict.ok) {
    ...
    routes.post('/api/revocations', async (request, response) => {
        const body = request.body;
        const revocation = (0, certificate_1.parseRevocation)(body.revocation);
        ...
        const verdict = await (0, certificate_1.verifyDetached)((0, certificate_1.canonicalRevocation)(revocation), body.signature ?? '', [armored]);
        ...
        response.json({ ok: true, canonical: (0, certificate_1.canonicalRevocation)(revocation) });
    });
    /**
    ...
    routes.get('/api/certificates', async (request, response) => {
        ...
        const result = await (0, service_1.answer)(store, config, {
            hash: query.hash,
            hasher: query.hasher,
            fingerprint: query.fingerprint,
            depth: Number.isFinite(depth) ? Math.max(0, depth) : 0,
            via: (0, protocol_1.parseVia)(query.via, config.policy.maxViaLength),
        });
        if (query.format === 'bundle') {
            response.json({
                ...
                revocations: (await store.revocations({ hash: query.hash, hasher: query.hasher })).map((stored) => ({
    ...
    routes.get('/api/federation', async (_request, response) => {
        response.json((0, service_1.descriptorFor)(config, await store.counts()));
    });
    /**
    ...
    routes.get('/api/certificates/export', async (request, response) => {
        ...
        const after = (0, protocol_1.parseCursor)(query.since);
        ...
        response.json({
            ...
            entries: page.certificates
                .map((certificate) => (0, service_1.toEntry)(certificate, config.publicUrl))
    ...
    routes.post('/api/import', requireOperator, async (request, response) => {
        const bundle = (0, protocol_1.parseBundle)(request.body);
        if ('error' in bundle) {
            response.status(400).json({ error: bundle.error });
            return;
        }
        response.json(await (0, service_1.importBundle)(store, bundle, bundle.origin ?? 'pushed'));
    });
    ...
    routes.post('/api/peers/announce', async (request, response) => {
        ...
        const result = await (0, service_1.announcePeer)(store, config, url);
        if ('error' in result) {
    ...
    const PEER_STATUSES = ['seed', 'active', 'candidate', 'blocked'];
    routes.post('/api/peers/status/:status', requireOperator, async (request, response) => {
        ...
        if (!existing)
            await store.upsertPeer(url, '', status);
        else
            await store.setPeerStatus(url, status);
        response.json({ ok: true, url, status });
    });
    routes.post('/api/peers/pull', requireOperator, async (request, response) => {
        const { url } = request.body;
        response.json({
            pulled: url ? [await (0, service_1.pullFrom)(store, config, url)] : await (0, service_1.pullFromAll)(store, config),
        });
    });
    routes.post('/api/peers/discover', requireOperator, async (request, response) => {
        ...
        response.json({ learned: await (0, service_1.discoverThrough)(store, config, url) });
    });
    // -------------------------------------------------------------- trust lists --
    ...
    routes.get('/api/trusted', requireUser, async (request, response) => {
        const { hasher } = request.query;
        const hashes = await store.trustedHashes(currentUser(request).id, hasher);
        const revocations = await store.revocations({ hasher });
        const withdrawn = new Set(revocations.map((stored) => `${stored.revocation.fingerprint} ${stored.revocation.hash} ${stored.revocation.hasher}`));
        response.json({
            hashes: hashes.filter((row) => !withdrawn.has(`${row.fingerprint} ${row.hash} ${row.hasher}`) ||
                Date.parse(row.asserted) >
                    Date.parse(revocations.find((stored) => stored.revocation.fingerprint === row.fingerprint &&
                        stored.revocation.hash === row.hash &&
                        stored.revocation.hasher === row.hasher)?.revocation.revoked ?? '1970-01-01T00:00:00Z')),
        });
    });
    return routes;
}

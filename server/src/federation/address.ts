...
export interface UrlOptions {
  /**
   * Whether a query string is allowed.
   *
   * Two different things are checked by this function.  A *peer* URL is an
   * identity — the name one node knows another by — and must be written one
   * way, so a query on it is meaningless and refused.  A *request* URL is that
   * identity plus an endpoint, and of course carries one.  Conflating them
   * makes every federated GET fail the moment it has parameters.
   */
  allowQuery?: boolean
}

/** Normalise for comparison: a peer is one node however its URL is written. */
export function normalizeUrl(raw: string, options: UrlOptions = {}): string | { error: string } {
  let url: URL
  try {
    url = new URL(raw)
  } catch {
  ...
  if (url.username || url.password) return { error: 'a peer URL must carry no credentials' }
  if (url.hash) return { error: 'a peer URL must carry no fragment' }
  if (url.search && !options.allowQuery) return { error: 'a peer URL must carry no query' }
...
function expandIPv6(address: string): number[] | null {
  ...
  const parse = (part: string): string[] => (part.length === 0 ? [] : part.split(':'))
  ...
  for (const part of left) push(part)
  ...
  if (tail === undefined) return filled === 8 ? groups : null
...
export function isBlockedAddress(address: string): boolean {
  ...
  if (family === 4) return isBlockedIPv4(address.split('.').map(Number))
  if (family === 6) {
 ...
 */
export async function checkPeerUrl(
  raw: string,
  policy: Pick<Policy, 'allowPrivate'>,
  options: UrlOptions = {},
): Promise<AddressCheck | { error: string }> {
  ...
  if (policy.allowPrivate) return { url: normalized, addresses: [] }

  const host = url.hostname.replace(/^\[|\]$/g, '')
  // A literal address never reaches the resolver, so it has to be checked here
  // or `http://127.0.0.1/` walks straight through.
  if (isIP(host)) {
    if (isBlockedAddress(host)) return { error: `refusing a non-public address (${host})` }
  ...
  if (resolved.length === 0) return { error: `cannot resolve ${host}` }
